...
def _api_text(value):
    """Return a stable JSON string for fields decoded as String on iOS."""
    return "" if value is None else str(value)

def _api_vehicle(v):
    dk=v.get("datova_kostka") or {};basic=dk.get("basic") or {}
    return {"id":_api_text(v.get("id")),"spz":_api_text(v.get("spz")),"vehicle_number":_api_text(v.get("vehicle_id")),"vin":_api_text(v.get("vin")),"brand":_api_text(basic.get("brand") or v.get("brand")),"model":_api_text(basic.get("model") or v.get("model") or v.get("name")),"year":_api_text(basic.get("year") or v.get("year")),"status":_api_text(v.get("status") or 'V provozu'),"km":_api_text(v.get("km")),"stk_until":_api_text(v.get("stk_until") or basic.get("inspection_until")),"vignette_until":_api_text(v.get("vignette_until")),"liability_until":_api_text(v.get("liability_until")),"casco_until":_api_text(v.get("casco_until")),"assistance_until":_api_text(v.get("assistance_until")),"next_service_date":_api_text(v.get("next_service_date")),"next_service_km":_api_text(v.get("next_service_km")),"fuel":_api_text(basic.get("fuel") or v.get("fuel")).upper(),"engine_type":_api_text(basic.get("engine_type") or v.get("engine_type")),"engine_capacity":_api_text(basic.get("engine_capacity") or v.get("engine_volume")),"power_kw":_api_text(basic.get("power_kw") or v.get("engine_power")),"emission":_api_text(basic.get("emission") or v.get("emission_class")),"photo_url":url_for('static',filename='images/'+v.get('photo'),_external=True) if v.get('photo') else '',"alerts":_api_alerts_for_vehicle(v)}
